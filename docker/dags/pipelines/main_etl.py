...
def main_etl():
    logger.info("=" * 50)
    logger.info(
        f"Iniciando ETL em {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    logger.info("=" * 50)

    directories = setup_all_directories()
    raw_dir = directories["raw_data_dir"]
    processed_dir = directories["processed_data_dir"]
    ...
    try:
        ...
        try:
            ...
            if not noticias:
                raise Exception("API retornou lista vazia")
        except Exception as e:
            logger.warning(f"Erro na API: {e}. Carregando mock")
            mock_path = os.path.join("mock_data", "mock_news_data.json")
        ...
        logger.info(
            f"Extract finalizado com sucesso: {len(noticias)} notícias extraídas"
        )
    except Exception as e:
    ...
    try:
        ...
        logger.info(
            f"Transform finalizado com sucesso: {len(processed_data)} notícias relevantes identificadas"
        )
    except Exception as e:
    ...
    try:
        ...
        if not bucket_name:
            raise ValueError(
                "Nome do bucket S3 não encontrado. Configure a variável S3_BUCKET_NAME no arquivo .env"
            )
        ...
        logger.info(
            f"Load finalizado com sucesso: dados enviados para {bucket_name}/{object_name_timestamped}"
        )
    except Exception as e:
    ...
    logger.info("=" * 50)
    logger.info(
        f"Pipeline ETL concluído com sucesso em {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    logger.info("=" * 50)
    return True
